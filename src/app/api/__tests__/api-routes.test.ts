...
jest.mock('@/lib/supabase/server', () => ({
  createRouteClient: jest.fn(() => Promise.resolve({
    ...
    from: jest.fn(() => ({
      select: jest.fn(() => ({
        eq: jest.fn(() => ({
          single: jest.fn(() => Promise.resolve({ data: { subscription_tier: 'free' }, error: null })),
...
// Importar a função a ser testada diretamente
jest.mock('@/app/api/analyze/route', () => ({
...
// Importar a função POST diretamente
