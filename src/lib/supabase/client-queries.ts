...
export function createClientSupabaseClient() {
  console.log('Criando cliente Supabase com URL:', !!process.env.NEXT_PUBLIC_SUPABASE_URL ? 'definida' : 'indefinida')
...
export async function checkMessageLimit(): Promise<{ limitReached: boolean, error: string | null, plan: string | null }> {
  ...
  try {
    ...
    // Obter perfil com contagem de mensagens e plano
    ...
    if (error) {
      ...
      return { limitReached: false, error: 'Erro ao verificar perfil', plan: null }
    }
    ...
    // No plano gratuito, verificar se atingiu o limite de 30 mensagens
...
export async function incrementMessageCount(): Promise<{ success: boolean, error: string | null }> {
  ...
  try {
    ...
    // Incrementar contagem na tabela de perfis
    ...
    return { success: false, error: 'Erro ao incrementar contagem de mensagens' }
  }
}
